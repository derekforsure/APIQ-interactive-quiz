...
export default function StudentManagementPage() {
  ...
  async function fetchDepartments() {
    ...
    // Handle the nested data structure from groups API
    const groupsData = data.data?.data || data.data || [];
    setDepartments(Array.isArray(groupsData) ? groupsData : []);
  }
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    const studentDataToSend = {
      ...newStudent,
      department_id: newStudent.department_id ? parseInt(newStudent.department_id, 10) : null,
    };
                ...
                {students.map((student) => (
                  <tr key={student.id} className={`transition-colors ${student.is_active === 0 ? 'bg-gray-50' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      {student.department || <span className="text-gray-400 italic">No Group</span>}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
